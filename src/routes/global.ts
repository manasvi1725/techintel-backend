...
router.get("/", async (_req: Request, res: Response) => {
  try {
    ...
    console.log(
      "📦 [/api/global] latest_json keys:",
      Object.keys(doc.latest_json ?? {})
    )

    if (!doc.latest_json?.global) {
      console.error(
        "❌ [/api/global] latest_json.global is MISSING",
        doc.latest_json
      )
    ...
    console.log(
      "📊 [/api/global] global section keys:",
      Object.keys(doc.latest_json.global)
    )
    ...
    if (investments?.countries) {
      console.log(
        "🌐 [/api/global] investment countries:",
        Object.keys(investments.countries)
      )
    } else {
