...
export async function refreshTechnologies() {
    await connectDB();
    const techs = (await Technology.find({ name: { $not: /^__/ } }, { name: 1, last_refreshed_at: 1 }).lean());
    const stale = techs.filter((tech) => {
        if (!tech.last_refreshed_at)
            return true;
    ...
    for (let i = 0; i < stale.length; i += MAX_CONCURRENT) {
        ...
        await Promise.all(batch.map(async (tech) => {
            ...
            const res = await fetch(`${ML_TRIGGER_URL}/internal/run?tech=${encodeURIComponent(tech.name)}`, {
                method: "POST",
                headers: {
            ...
            await Technology.findOneAndUpdate({ name: tech.name }, {
                latest_json: payload,
